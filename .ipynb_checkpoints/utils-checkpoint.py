...
def save_crop(base_dir, image_name, index, crop):
    ...
    new_dataset = rasterio.open(filename, 'w', height=crop.shape[0], width=crop.shape[1], count=1, dtype=crop.dtype,
                                driver='GTiff')
...
def predict_file(model, csv_file, root_dir, device, iou_threshold=0.1):
    model.eval()
    ...
    for index, prediction in enumerate(prediction_list):
        ...
        if len(prediction.label.unique()) > 1:
            prediction = predict.across_class_nms(prediction, iou_threshold=iou_threshold)

# Date:2019.04.10
# Author: DuncanChen
# Repo: https://github.com/DuncanChen2018/shp2coco/issues/5
...
class GeoTiff(object):
    ...
    def clip_tif_with_grid(self, clip_size, begin_id, out_dir):
        ...
        count = 1
        for i in np.arange(0, row_num - 1, 0.5):
            for j in np.arange(0, col_num - 1, 0.5):
                ...
                try:
                    save_path = os.path.join(out_dir, '%d.tif' % (begin_id + count))
    ...
    def clip_mask_with_grid(self, clip_size, begin_id, out_dir):
        ...
        for i in np.arange(0, row_num-1, 0.5):
            for j in np.arange(0, col_num-1, 0.5):
                clipped_image = np.array(self.mask[0, int(i * clip_size): int((i + 1) * clip_size),
                                         int(j * clip_size): int((j + 1) * clip_size)])
                ...
                save_path = os.path.join(out_dir, f'{begin_id+c}.tif')
                save_image_with_georef(clipped_image, gtiffDriver,
                                       self.dataset, j * clip_size, i * clip_size, save_path)
                count += 1
                # for id in range(len(ins_list)):
                #     bg_img = np.zeros(clipped_image.shape).astype(np.int8)
                #     if (len(clipped_image[clipped_image==ins_list[id]])/len(self.mask[self.mask==ins_list[id]])<0.7):
                #         continue
                #     # if len(clipped_image[clipped_image == ins_list[id]]) != len(self.mask[self.mask == ins_list[id]]):
                #     #     continue
                #     if ins_list[id] > 0:
                #         bg_img[np.where(clipped_image == ins_list[id])] = 255
                #     try:
                #         save_path = os.path.join(out_dir, '%d_%s_%d.tif' % (begin_id + c, 'uchastok', l))
                #         l += 1
    ...
    def mask_tif_with_shapefile(self, shapefile_path, label=255):
        ...
        if dataSource is None:
            print("Can not open:", shapefile_path)
    ...
    def clip_tif_and_shapefile(self, clip_size, begin_id, shapefile_path, out_dir):
        ...
        pic_id = self.clip_tif_with_grid(clip_size=clip_size, begin_id=begin_id, out_dir=out_dir + '/uchastok_2021')
        return pic_id
...
class GeoShaplefile(object):
    ...
    def open_shapefile(self):
        driver = ogr.GetDriverByName('ESRI Shapefile')
        dataSource = ogr.Open(self.file_path, 0)
        if dataSource is None:
            raise IOError('could not open!')
        gtiffDriver = gdal.GetDriverByName('GTiff')
        if gtiffDriver is None:
...
if __name__ == '__main__':
    root = r'/media/deadman445/disk/fff/'
    img_path = '/Raster'
    shp_path = '/Razmetka'
    clip_from_file(1024, root, img_path, shp_path)
